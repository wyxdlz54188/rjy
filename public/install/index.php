...
<div class="panel panel-primary">
	...
	<thead>
		<tr>
			<th><br>1.此程序为免费开源程序！<br>2.请勿用此程序上传任何违反国家法律的内容！<br>3.此程序提供在线更新服务，使用更新前请先自行备份数据！<br>4.对于本软件保证不含有任何恶意破坏您的计算机资源（包括文档、程序和其他数据）的功能设置。对于您在使用此软件过程中而产生的利润损失、可用性消失、商业中断，或任何形式的间接、特别、意外或必然的破坏，或任何其他方的索赔概不负责，即使已事先被告知此类事有可能发生。<br></th>
		</tr>
	</thead>	
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=mysqli_connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(mysqli_query($con, "select * from ".$dbconfig['dbqz']."_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过软件源安装系统</div>
...
';
		if(!$con=mysqli_connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			echo '<div class="alert alert-warning">MySQL数据库版本太低，需要MySQL 5.6或以上版本！</div>';
		}elseif(file_put_contents('../../.env',$config1)){
...
<?php
	...
	clearpack();
	echo '<div class="alert alert-info"><font color="green">安装完成！管理账号和密码是:nk8686/123456</font><br/><br/><a href="/FRKToHDckx.php">>>后台管理</a><hr/>更多设置选项请登录后台管理进行修改！<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 install.lock 文件！</font></div>';
?>
	</div>
</div>
